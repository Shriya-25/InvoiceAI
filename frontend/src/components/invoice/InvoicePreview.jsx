    ...
    <div className="invoice-paper bg-white rounded-xl overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.08)] border border-[#E5E7EB]">
      ...
      <div className="bg-[#1A998F] text-white px-8 py-6">
        <div className="flex justify-between items-start">
          ...
          <div className="text-right">
            ...
            <p className="text-teal-100 text-sm mt-1">#{invoice.invoiceNumber || 'INV-001'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-8 py-5 grid grid-cols-2 gap-6 bg-white">
        <div>
          ...
          <p className="font-bold text-[#102E3C] text-sm">{invoice.client?.name || <span className="text-[#D1D5DB]">Client Name</span>}</p>
        ...
        <div className="text-right">
          ...
          {[
            ...
            <div key={label} className="flex justify-end gap-3 text-xs mb-1">
              ...
              <span className="font-semibold text-[#102E3C] min-w-[80px] text-right">{value}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-8 pb-4">
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-[#F4F7F6] rounded-lg">
          ...
          <tbody>
            {(invoice.items || []).map((item, i) => (
              <tr key={item.id || i} className="border-b border-[#F3F4F6] last:border-0">
                <td className="py-3 px-3 text-[#102E3C]">{item.description || <span className="text-[#D1D5DB]">Service description</span>}</td>
                ...
                <td className="py-3 px-3 text-right font-semibold text-[#102E3C]">
      ...
      <div className="px-8 pb-6">
        <div className="ml-auto max-w-[220px]">
          ...
          <div className="bg-[#1A998F] text-white rounded-lg px-3 py-2.5 flex justify-between items-center mt-2">
        ...
        <div className="px-8 py-5 border-t border-[#F3F4F6] bg-[#FAFAFA]">
          {invoice.paymentTerms && (
            <div className="mb-3">
              ...
              <p className="text-xs text-[#102E3C]">{invoice.paymentTerms}</p>
            </div>
          )}
          {invoice.notes && (
            <div>
      ...
      {/* Watermark footer */}
      <div className="px-8 py-3 bg-[#F4F7F6] border-t border-[#E5E7EB] flex justify-between items-center">
